...
from jet_detectors import jet_detector
...
def model(jets, other_features, w_h, w_h2, w_h3, w_h4, w_o, b_h, b_h2, b_h3, b_h4, b_o, p_drop_input, p_drop_hidden):
    X = dropout(other_features, p_drop_input)
    h = tanh(T.dot(other_features, w_h) + b_h)

    h = dropout(h, p_drop_hidden[0])
    h2 = tanh(T.dot(h, w_h2) + b_h2)

    jet_detectors, jet_params = jet_detector(jets)

    h2 = dropout(h2, p_drop_hidden[1])
    h3 = tanh(T.dot(T.concatenate([h2, jet_detectors], axis=1), w_h3) + b_h3)
...
noise_h, noise_h2, noise_h3, noise_h4, noise_py_x, jet_params = model(jets, other_features, w_h, w_h2, w_h3, w_h4, w_o, b_h, b_h2, b_h3, b_h4, b_o, 0.8, [0.5,0.5,0.5,0.5])

h, h2, h3, h4, py_x, jet_params = model(jets, other_features, w_h, w_h2, w_h3, w_h4, w_o, b_h, b_h2, b_h3, b_h4, b_o, 0., [0.,0.,0.,0.])
...
for i in range(1000):
    for start, end in zip(range(0, len(trX), 100), range(100, len(trX), 100)):
        cost = train(trX[start:end, 6:22].T, trX[start:end, range(6) + range(22, 28)], trY[start:end])
        batches_seen += 1
        # Learning rate decay
        lr.set_value(floatX(np.amax((init_lr / (decay_factor ** batches_seen), min_lr))))
    pred = predict(vX[start:end, 6:22].T, vX[start:end, range(6) + range(22, 28)])
